...
def find_maximum_matching(structure, matching):
    path = find_augmenting_path(structure, matching)
    if path:
        new_matching = augment_path(matching, path)
        return find_maximum_matching(structure, new_matching)
    else:
        return matching

def find_augmenting_path(structure, matching):
    forest = []
...
if __name__ == "__main__":
    ...
    matching = Match.from_structure(structure)
    print(matching.unmatched_nodes())
    print(matching.freenodes)
    for atom in structure.graph:
        if atom.__repr__() == 'N_8':
            for orbital_name, orbital in atom.valence_shell.orbitals.items():
                print(orbital, orbital.electrons)
